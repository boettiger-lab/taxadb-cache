...
in_url <- "ftp://ftp.ncbi.nlm.nih.gov/pub/taxonomy/taxdump_archive/taxdmp_2019-09-01.zip"
in_file <- file.path("2019", basename(in_url))
curl::curl_download(in_url,  in_file)
code <- c("R/ncbi.R", "R/helper-routines.R", "make.R")
output_paths <- c(dwc = "2019/dwc_ncbi.tsv.gz",
                  common = "2019/common_ncbi.tsv.gz")
...
prov::write_prov(data_in = in_file, code = code, 
                 data_out =  unname(output_paths), 
                 prov="prov-2019.json", append=TRUE)
...
#in_url <- "http://www.catalogueoflife.org/DCA_Export/zip-fixed/2019-09-01-archive-complete.zip"
in_url <- paste0("http://www.catalogueoflife.org/DCA_Export/zip-fixed/", 2019, "-annual.zip")
in_file <- "2019/col-2019.zip"
curl::curl_download(in_url, in_file)
code <- c("R/col.R","R/helper-routines.R", "make.R")
output_paths <- c(dwc = "2019/dwc_col.tsv.gz",
                  common = "2019/common_col.tsv.gz")

preprocess_col(url = in_url, output_paths = output_paths)

source(system.file("examples/minio_store.R", package="prov"))
minio_store(c(in_file, code, output_paths))
prov::write_prov(data_in = in_file, code = code, data_out =  unname(output_paths), prov="prov-2019.json", append=TRUE)
...
in_file <- "2019/gbif-backbone-current.zip"
curl::curl_download(in_url, in_file)
code <- c("R/gbif.R","R/helper-routines.R", "make.R")

output_paths = c(dwc = "2019/dwc_gbif.tsv.gz", common = "2019/common_gbif.tsv.gz")
...
in_file <- "2019/itisSqlite.zip"
curl::curl_download(in_url, in_file)

output_paths = c(dwc = "2019/dwc_itis.tsv.gz", common = "2019/common_itis.tsv.gz")
